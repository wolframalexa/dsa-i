...
void insertionSort(list<Data *> &l)
{
  ...
  {
    Data* temp = *it2;
    for (it = next(it2,1); (it != begin) && (comparatorT12(temp, *(prev(it, 1)))); it--) {
      *it = *(prev(it, 1));
    }
    *it = temp;
    cout << (*it2)->lastName << " " << (*it2)->firstName << " " << (*it2)->ssn << "\n";
    begin++;
    break;
  }
}
...
{
  int i = 0;
  for (list<Data *>::iterator it = l.begin(); i < 500000000; it++)
  {
    string str = (*it)->ssn;
  ...
  for (list<Data *>::iterator it = l.begin+i ; i < 1000000000; it++)
  {
    string str = (*it)->ssn;
    SSNInt2[i] = str[0]*100000000+str[1]*10000000+str[2]*1000000+str[4]*100000+str[5]*10000+str[7]*1000+str[8]*100+str[9]*10+str[10];
    i++;
  }
